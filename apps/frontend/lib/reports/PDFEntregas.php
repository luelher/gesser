...
class PDFEntregas extends sfTCPDF {
  ...
  public function MyHeader()
  {
    ...
    $this->Cell(0, 0, '', 'T', 0, 'C');
		...
		public function setHeaderData($ln='', $lw=0, $ht='', $hs='', $a='') {
			...
			$this->header_title = 'INFORME DE ENTREGAS';
    ...
    public function Body($desde, $hasta)
    {
      ...
      $entregas = Doctrine_Query::create()->from('Entregas')->where ('fecha >= ?', $desde)->addwhere('fecha <= ?', $hasta)->execute();
      
      if($entregas){
        $this->SetFont('helvetica', '', 7);
        foreach ($entregas as $entrega) {
          $servicio = $entrega->getServicio();
          $y = $this->getY();
          $this->Cell(20, 4, $servicio->getCliente(),false);
          $this->Cell(20, 4, date('d/m/Y',strtotime($entrega->getFecha())),false);
          ...
          $this->SetFont('helvetica', '', 7);
          $this->MultiCell(60, 4, $entrega->getObservacion(),false);
        }
      }else{
