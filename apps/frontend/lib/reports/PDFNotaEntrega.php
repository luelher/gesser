...
class PDFNotaEntrega extends sfTCPDF {
  ...
  public function MyHeader()
  {
    ...
    $this->Cell(0, 0, '', 'T', 0, 'C');
		...
		public function setHeaderData($ln='', $lw=0, $ht='', $hs='', $a='') {
			...
			$this->header_string = 'NOTA DE ENTREGA';
      $this->author = $a;
		}
    ...
    public function Body($id)
    {
      ...
      $entregas = Doctrine::getTable('Entregas')->findOneBy('id', $id);
      $servicio = Doctrine::getTable('Servicios')->findOneBy('id', $entregas->getServiciosId());
      if($servicio){

        $this->SetFont('helvetica', '', 12);
        
        $this->setXY(15,35);
        $this->Cell(80, 0, 'Mediante la presente hacemos constar que la Sr/Sra: '.trim($servicio->getNombre()).', ');
        $this->setXY(15,40);
        $this->Cell(80, 0, 'C.I Nro. '.$servicio->getCliente().', en Fecha '.date('d/m/Y',strtotime($entregas->getFecha())));
        $this->setXY(15,45);
        $this->Cell(80, 0, 'Le fue entregado el/los siguiente(s) equipos:');
        $this->setXY(30,55);
        $this->SetFont('helvetica', 'B', 14);
        $this->Cell(120, 0,'- '.$servicio->getNomart());
        $this->setXY(30,60);
        $this->SetFont('helvetica', 'B', 12);
        $this->Cell(120, 0,'( Factura:  '.$servicio->getFactura().'  , De Fecha: '.$servicio->getFactFecEmis().' )');
        $this->SetFont('helvetica', '', 12);
        $this->setXY(15,70);
        $this->MultiCell(180, 15, 'Observaciones: '.$entregas->getObservacion(), false, 'L');
